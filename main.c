...
int main(void) {
    ...
    if (!train_images || !test_images) {
        fprintf(stderr, "Error: Failed to load image data\n");
...

    {
        ...
        if (!train_labels_file || !test_labels_file) {
            fprintf(stderr, "Error: Failed to load label data\n");
            arena_destroy(perm_arena);
            return 1;
        }
        ...
        for (u32 i = 0; i < 60000; i++) {
            ...
            if (num >= 10) {
                fprintf(stderr, "Error: Invalid training label %u at index %u (expected 0-9)\n", num, i);
                arena_destroy(perm_arena);
                return 1;
            }
        ...
        for (u32 i = 0; i < 10000; i++) {
            ...
            if (num >= 10) {
                fprintf(stderr, "Error: Invalid test label %u at index %u (expected 0-9)\n", num, i);
                arena_destroy(perm_arena);
                return 1;
            }
...
matrix* mat_create(mem_arena* arena, u32 rows, u32 cols) {
    ...
    if (rows > 0 && cols > UINT32_MAX / rows) {
        fprintf(stderr, "Error: Matrix dimensions too large (%u x %u)\n", rows, cols);
        return NULL;
    }

    u64 total_size = (u64)rows * cols;
    if (total_size > SIZE_MAX / sizeof(f32)) {
    ...
    mat->rows = rows;
    mat->cols = cols;
    mat->data = PUSH_ARRAY(arena, f32, total_size);

    if (!mat->data) {
        fprintf(stderr, "Error: Failed to allocate matrix data (%llu elements)\n",
...
matrix* mat_load(mem_arena* arena, u32 rows, u32 cols, const char* filename) {
    ...
    if (!mat) {
        fprintf(stderr, "Error: Failed to create matrix for file '%s'\n", filename);
        return NULL;
    }
    ...
    u64 size = MIN((u64)file_size, sizeof(f32) * (u64)rows * cols);

    u64 read = fread(mat->data, 1, size, f);
    if (read != size) {
        fprintf(stderr, "Warning: Expected to read %llu bytes from '%s', but read %llu\n",
                (unsigned long long)size, filename, (unsigned long long)read);
    }
